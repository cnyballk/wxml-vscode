var debug =
  process.env.DEBUG_JSBEAUTIFY || process.env.JSBEAUTIFY_DEBUG
    ? function() {
...
var interpret = (exports.interpret = function(argv, slice) {
  ...
  var configHome = verifyExists(
    path.join(getUserHome() || '', '.jsbeautifyrc')
  );
  ...
  if (outfile) {
    ...
    if (isFileDifferent(outfile, pretty)) {
      try {
        ...
        logToStdout(
          'beautified ' + path.relative(process.cwd(), outfile),
          config
        );
      } catch (ex) {
        onOutputError(ex);
      ...
      logToStdout(
        'beautified ' + path.relative(process.cwd(), outfile) + ' - unchanged',
        config
      );
    }
  } else {
...
function getOutputType(outfile, filepath, configType) {
  if (outfile && /\.(js|css|html)$/.test(outfile)) {
    ...
    throw 'Could not determine appropriate beautifier from file paths: ' +
      filepath;
  }
}
